...
description: Research agent that runs before virtually any build task. Auto-triggers before landing pages, funnels, campaigns, email sequences, product launches, ad copy, and strategy sessions. Manual trigger any time the user says "research X" or "what are competitors doing with X." Runs deep competitive, customer, market, and product research using Claude Code's built-in web search. Outputs a structured brief that feeds directly into the build.
---
...
Nothing to install and no API key. This skill uses Claude Code's built-in **WebSearch** (live web queries) and **WebFetch** (pull and read a specific page). If WebSearch is not available in your environment, say so, then fall back to WebFetch on known URLs plus what you already know, and flag that the research is lighter than a full run.
...
Research questions (run with WebSearch):
...
**How to run each track:** use the **WebSearch** tool for the questions above, one focused query at a time, and follow the trail across several results. When a result points to a specific competitor page, review, or forum thread, use **WebFetch** to pull that page and read it directly. Synthesize across sources. Do not stop at the first hit, and do not summarize a single page as if it were the whole picture.

### Brief Mode

Single WebSearch query. One targeted question. Done in under 5 minutes.
